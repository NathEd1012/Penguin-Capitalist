    ...
    for symbol in SYMBOLS:
        try:
            ...
            if not quote or quote.bid_price is None or quote.ask_price is None:
                print(
                    f"{symbol:<10} {'N/A':<12} {'N/A':<12} {'N/A':<10} {'N/A':<12} ❌ NO QUOTE"
                )
            ...
            print(
                f"{symbol:<10} ${bid:<11.2f} ${ask:<11.2f} ${spread:<9.2f} {age_str:<12} {status}"
            )

        except Exception as e:
            print(
                f"{symbol:<10} {'ERROR':<12} {'ERROR':<12} {'ERROR':<10} {'N/A':<12} ❌ {type(e).__name__}"
            )
            missing_symbols.append(symbol)
...
def monitor_prices(client, duration_minutes=5, interval_seconds=30):
    ...
    stuck_symbols = {
        s: no_change_count[s] for s in no_change_count if no_change_count[s] > 0
    }
...
def check_quote_freshness():
    ...
    if market_open is False:
        ...
        if clock:
            ...
            print(
                f"📅 Next market open: {next_open.strftime('%Y-%m-%d %H:%M:%S %Z')} ({hours:.1f} hours from now)"
            )
        print(
    ...
    print(f"\nSummary:")
    print(
        f"  Fresh quotes:   {len(SYMBOLS) - len(stale_symbols) - len(missing_symbols)}"
    )
